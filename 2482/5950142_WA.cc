...
void insert(Node *pnode, int left, int right, int count, int *pmost, int descent)
{
    if ( pnode == NULL || left > right ) return ;
    if ( pnode->left == left && pnode->right == right ) { 
	pnode->count += count;
	if ( count > 0 ) query(pnode, pmost, descent);
	return ;
    }
    int nc = pnode->count + descent;
    int mid = (pnode->left + pnode->right) >> 1;
    insert( pnode->plc, left, min(mid, right), count, pmost, nc );
    insert( pnode->prc, max(mid+1, left), right, count, pmost, nc );
}
...
//	    else stars[xi>>1].xls = i;
	...
	for ( int i = 0; i < xn; ++i ) {
		...
		insert( proot, stars[si].yls, stars[si].yle, -1*stars[si].c, &most, 0);
	    } else {
		insert( proot, stars[si].yls, stars[si].yle, stars[si].c, &most, 0);
	    }
	}
	cout << most << endl;
	reset_node();
