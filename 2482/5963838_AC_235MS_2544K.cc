...
int main()
{
	...
	for ( int i = 0; i < n; ++i ) {
	    ...
	    xlabels[xn].value = stars[i].x - W + OFFSET; 
	    xlabels[xn].index = xn;
	    xn++;
	    xlabels[xn].value = stars[i].x - OFFSET;	
	    xlabels[xn].index = xn;
	    xn++;

	    ylabels[yn].value = stars[i].y - H + OFFSET;
	    ylabels[yn].index = yn;
	    yn++;
	    ylabels[yn].value = stars[i].y - OFFSET;
	    ylabels[yn].index = yn;
	    yn++;
	}
